-- DANGER: This script drops existing tables to enforce the new UUID-based schema.
-- Run this only if you are okay with losing existing testing data in 'products', 'orders', etc.

DROP TABLE IF EXISTS order_items CASCADE;
DROP TABLE IF EXISTS orders CASCADE;
DROP TABLE IF EXISTS products CASCADE;
DROP TABLE IF EXISTS inquiries CASCADE;
-- We do NOT drop profiles usually, but to ensure role column exists cleanly:
-- DROP TABLE IF EXISTS profiles CASCADE; 

-- Enable UUID extension
create extension if not exists "uuid-ossp";

-- 1. Profiles (Update/Create)
...
-- Idempotent column check for role
do $$
begin
...
drop policy if exists "Admins can see all order items" on order_items;
